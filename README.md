# European anthropic megalithic transport
...
Current build: **v0.4.9** (13 September 2026). 43 hauls. The map has a **Not Stonehenge** toggle so the four Salisbury Plain lines do not swamp western Europe.

## v0.4.9 notes

- Legend keys match the map: filled circle = monument, open circle = source, small arrowhead = haul direction (no diamond).

## v0.4.8 notes

- Arrowheads shrink with zoom (and stay small on mobile) so close-ups stay readable and tappable.

## v0.4.7 notes

- Smaller arrowheads (especially on mobile) so they do not obscure monuments or steal taps; arrows are non-interactive.
...
- Headline uses **anthropic** for findability: European anthropic megalithic transport.
- Distance histogram restored as a clickable vertical chart (tap a km band to filter).
...
Click a line, a point, or a row in the side list. Monument = filled circle; source = open circle. Each haul has a stable id (`#SH-altar`) shared by the map and the gazetteer page.
...
- Distance histogram is a real chart: vertical bars, labelled bands, click a bar to filter the map to that km range.
